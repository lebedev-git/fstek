// Общие определения MCP-сервера трекера ФСТЭК №117.
// buildServer() создаёт свежий McpServer со всеми инструментами — используется
// и stdio-сервером (server.mjs), и HTTP-сервером (http-server.mjs, свежий на каждый запрос).
...
export const INSTRUCTIONS = `Ты — аудитор соответствия ГИС Приказу ФСТЭК №117.
...
// Собрать свежий сервер со всеми инструментами и ресурсом протокола.
